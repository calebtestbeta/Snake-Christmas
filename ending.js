window.Ending = (() => {
    // 聖誕祝福與屬靈勉勵訊息
    const blessingLines = {
        // 高信心成長
        highFaith: "你的信心如磐石般穩固，神必賜福你一切的道路。「你們祈求，就給你們；尋找，就尋見；叩門，就給你們開門。」- 馬太福音 7:7",
        
        // 高愛心成長  
        highLove: "愛心滿溢的你，正如基督愛我們一樣愛人。「我們愛，因為神先愛我們。」- 約翰一書 4:19",
        
        // 高盼望成長
        highHope: "你的心充滿盼望，如同仰望晨星的人。「但那等候耶和華的必從新得力。他們必如鷹展翅上騰。」- 以賽亞書 40:31",
        
        // 高平安成長
        highPeace: "平安如江河在你心中流淌，神的同在與你同行。「你們要將一切的憂慮卸給神，因為他顧念你們。」- 彼得前書 5:7",
        
        // 高喜樂成長
        highJoy: "你的喜樂是從上頭來的恩賜，讓人看見神的榮光。「你們要靠主常常喜樂，我再說，你們要喜樂。」- 腓立比書 4:4",
        
        // 高讚美成長
        highPraise: "你的讚美如香上升到神面前，蒙神悅納。「凡有氣息的都要讚美耶和華！」- 詩篇 150:6",
        
        // 高智慧成長
        highWisdom: "你行走在智慧的道路上，腳步穩健不偏離。「敬畏耶和華是智慧的開端；認識至聖者便是聰明。」- 箴言 9:10",
        
        // 聖誕特殊祝福
        christmasBlessing: "願聖誕的平安喜樂常與你同在，如同當年天使報佳音般的美好。「因有一嬰孩為我們而生；有一子賜給我們。」- 以賽亞書 9:6",
        
        // 多重詞句完成
        multiPhrase: "你收集了許多美好的詞句，願這些祝福如種子在心中發芽成長。「神能照著運行在我們心裡的大力充充足足地成就一切。」- 以弗所書 3:20",
        
        // 均衡成長
        balanced: "你的屬靈生命成長均衡，如樹栽在溪水旁。「他要像一棵樹栽在溪水旁，按時候結果子，葉子也不枯乾。」- 詩篇 1:3",
        
        // 初次體驗
        beginner: "每一個美好的開始都是神恩典的記號，願你在信仰路上滿有盼望。「應當一無掛慮，只要凡事藉著禱告、祈求，和感謝，將你們所要的告訴神。」- 腓立比書 4:6",
        
        // 完全沒有收穫時的鼓勵
        noHarvest: "沒關係！每一次嘗試都是成長的開始。「凡勞苦擔重擔的人可以到我這裡來，我就使你們得安息。」- 馬太福音 11:28",
        
        // 豐盛收穫
        abundant: "你收穫滿滿的祝福，如同慈愛的神賜給兒女好東西。「你們雖然不好，尚且知道拿好東西給兒女；何況你們在天上的父，豈不更把好東西給求他的人嗎？」- 馬太福音 7:11"
    };

    // 聖誕節聖經經文祝福池
    const randomBlessings = {
        // 聖誕降生系列 - 基督誕生的奇蹟
        nativityScriptures: [
            "🎄「因有一嬰孩為我們而生；有一子賜給我們。政權必擔在他的肩頭上；他名稱為奇妙策士、全能的神、永在的父、和平的君。」- 以賽亞書 9:6",
            "⭐「必有童女懷孕生子，給他起名叫以馬內利（就是神與我們同在的意思）。」- 馬太福音 1:23",
            "👼「天使對他們說：不要懼怕！我報給你們大喜的信息，是關乎萬民的；因今天在大衛的城裡，為你們生了救主，就是主基督。」- 路加福音 2:10-11",
            "🌟「那星忽然在他們前頭行，直行到小孩子的地方，就在上頭停住了。他們看見那星，就大大的歡喜。」- 馬太福音 2:9-10"
        ],

        // 天使報佳音系列 - 天使的宣告
        angelicMessages: [
            "📯「在至高之處榮耀歸與神！在地上平安歸與他所喜悅的人！」- 路加福音 2:14",
            "🕊️「天使對馬利亞說：馬利亞，不要怕！你在神面前已經蒙恩了。」- 路加福音 1:30",
            "✨「馬利亞說：我心尊主為大；我靈以神我的救主為樂。」- 路加福音 1:46-47",
            "👼「主的使者在夢中向他顯現，說：大衛的子孫約瑟，不要怕！只管娶過你的妻子馬利亞來，因她所懷的孕是從聖靈來的。」- 馬太福音 1:20"
        ],

        // 光明與盼望系列 - 基督是世界的光
        lightAndHope: [
            "💡「那光是真光，照亮一切生在世上的人。」- 約翰福音 1:9",
            "🌅「在黑暗中行走的百姓看見了大光；住在死蔭之地的人有光照耀他們。」- 以賽亞書 9:2",
            "🔥「耶穌又對眾人說：我是世界的光。跟從我的，就不在黑暗裡走，必要得著生命的光。」- 約翰福音 8:12",
            "🌟「我們在東方看見他的星，特來拜他。」- 馬太福音 2:2"
        ],

        // 愛與救贖系列 - 神的愛透過基督彰顯
        loveAndSalvation: [
            "❤️「神愛世人，甚至將他的獨生子賜給他們，叫一切信他的，不至滅亡，反得永生。」- 約翰福音 3:16",
            "💝「惟有基督在我們還作罪人的時候為我們死，神的愛就在此向我們顯明了。」- 羅馬書 5:8",
            "🤗「我們愛，因為神先愛我們。」- 約翰一書 4:19",
            "💖「主所應許我們的就是永生。」- 約翰一書 2:25"
        ],

        // 平安與喜樂系列 - 聖誕帶來的平安
        peaceAndJoy: [
            "🕊️「我留下平安給你們；我將我的平安賜給你們。我所賜的，不像世人所賜的。你們心裡不要憂愁，也不要膽怯。」- 約翰福音 14:27",
            "🎵「你們要靠主常常喜樂。我再說，你們要喜樂。」- 腓立比書 4:4",
            "🌈「但願使人有盼望的神，因信將諸般的喜樂、平安充滿你們的心，使你們藉著聖靈的能力大有盼望！」- 羅馬書 15:13",
            "🎶「應當一無掛慮，只要凡事藉著禱告、祈求，和感謝，將你們所要的告訴神。」- 腓立比書 4:6"
        ]
    };

    // 聖誕節特別經文祝福
    const christmasSpecialBlessings = [
        "🎄「道成了肉身，住在我們中間，充充滿滿的有恩典有真理。我們也見過他的榮光，正是父獨生子的榮光。」- 約翰福音 1:14",
        "⭐「從他豐滿的恩典裡，我們都領受了，而且恩上加恩。」- 約翰福音 1:16",
        "👑「因為今天在大衛的城裡，為你們生了救主，就是主基督。」- 路加福音 2:11",
        "🕊️「願頌讚歸與我們主耶穌基督的父神！他在基督裡曾賜給我們天上各樣屬靈的福氣。」- 以弗所書 1:3"
    ];

    // 感恩讚美經文
    const gratitudeBlessings = [
        "🙏「凡有氣息的都要讚美耶和華！你們要讚美耶和華！」- 詩篇 150:6",
        "🎵「要以感謝為祭獻給神，又要向至高者還你的願。」- 詩篇 50:14",
        "✨「你們要給耶和華榮耀，因他的慈愛永遠長存。」- 詩篇 136:1",
        "💫「當稱謝進入他的門；當讚美進入他的院。當感謝他，稱頌他的名！」- 詩篇 100:4"
    ];

    // 特殊聖誕與信仰詞句的額外祝福
    const specialPhraseBlessings = {
        "聖誕快樂": "🎄 聖誕的真諦是愛與分享，願你的生命如星光般照亮他人。",
        "耶穌愛我": "✝️ 這是最寶貴的真理，願你在主的愛中得著完全的平安。",
        "哈利路亞": "🙏 你的讚美上達天庭，願神的榮耀充滿你的生命。",
        "以馬內利": "⭐ 神與我們同在，在每一天的生活中都經歷祂的同行。",
        "平安喜樂": "🕊️ 這是天父賜給兒女最美的禮物，願你時時經歷。",
        "恩典夠用": "💝 神的恩典在軟弱中顯得完全，祂的愛永不離棄。",
        "信望愛": "💖 這三樣是基督徒生命的根基，其中最大的是愛。",
        "報佳音": "📯 如同當年的天使，願你也成為傳遞福音的使者。",
        "平安夜": "🌙 在這寧靜的夜晚，感受神同在的美好與溫暖。"
    };

    // 分析屬靈成長狀況
    function analyze(spiritualStat, completedPhrases = []) {
        const faith = spiritualStat.faith || 0;
        const love = spiritualStat.love || 0;
        const hope = spiritualStat.hope || 0;
        const peace = spiritualStat.peace || 0;
        const joy = spiritualStat.joy || 0;
        const praise = spiritualStat.praise || 0;
        const wisdom = spiritualStat.wisdom || 0;
        const trust = spiritualStat.trust || 0;
        
        const totalGrowth = faith + love + hope + peace + joy + praise + wisdom + trust;
        
        // 檢查是否完全沒有收穫
        if (totalGrowth === 0 && completedPhrases.length === 0) {
            return "noHarvest";
        }
        
        // 檢查是否有聖誕相關詞句
        const hasChristmasPhrase = completedPhrases.some(phrase => 
            ['聖誕快樂', '平安夜', '報佳音'].includes(phrase)
        );
        
        // 檢查多重詞句完成
        const phraseCount = completedPhrases.length;
        
        // 分析邏輯 - 優先考慮特殊情況
        if (hasChristmasPhrase) {
            return "christmasBlessing";
        }
        
        if (phraseCount >= 5) {
            return "multiPhrase";
        }
        
        if (totalGrowth >= 80) {
            return "abundant";
        }
        
        if (totalGrowth < 20) {
            return "beginner";
        }
        
        // 根據最高的屬靈成長指標決定
        const maxValue = Math.max(faith, love, hope, peace, joy, praise, wisdom);
        
        if (faith === maxValue && faith >= 30) {
            return "highFaith";
        }
        if (love === maxValue && love >= 35) {
            return "highLove";
        }
        if (hope === maxValue && hope >= 25) {
            return "highHope";
        }
        if (peace === maxValue && peace >= 30) {
            return "highPeace";
        }
        if (joy === maxValue && joy >= 35) {
            return "highJoy";
        }
        if (praise === maxValue && praise >= 30) {
            return "highPraise";
        }
        if (wisdom === maxValue && wisdom >= 20) {
            return "highWisdom";
        }
        
        return "balanced";
    }

    // 獲取祝福訊息
    function getBlessingLine(tag) {
        return blessingLines[tag] || blessingLines.balanced;
    }

    // 獲取詞句特殊祝福
    function getPhraseBlessing(phrase) {
        return specialPhraseBlessings[phrase] || "";
    }

    // 智能隨機祝福選擇器
    function getRandomBlessingByCategory(spiritualStat, completedPhrases = []) {
        const faith = spiritualStat.faith || 0;
        const love = spiritualStat.love || 0;
        const hope = spiritualStat.hope || 0;
        const peace = spiritualStat.peace || 0;
        const joy = spiritualStat.joy || 0;
        const praise = spiritualStat.praise || 0;
        
        // 根據屬靈成長傾向選擇對應的聖經經文類別
        let selectedCategory = 'nativityScriptures'; // 預設聖誕降生經文
        
        // 檢查是否有聖誕相關詞句 - 優先聖誕降生經文
        const hasChristmasPhrase = completedPhrases.some(phrase => 
            ['聖誕快樂', '平安夜', '報佳音', '以馬內利'].includes(phrase)
        );
        
        if (hasChristmasPhrase) {
            selectedCategory = 'nativityScriptures';
        } else if (faith > 20 || praise > 15 || completedPhrases.some(p => ['哈利路亞', '讚美'].some(w => p.includes(w)))) {
            // 信心或讚美較高 - 天使報佳音
            selectedCategory = 'angelicMessages';
        } else if (love > 25 || completedPhrases.some(p => p.includes('愛'))) {
            // 愛心較高 - 愛與救贖經文
            selectedCategory = 'loveAndSalvation';
        } else if (hope > 15 || joy > 20) {
            // 盼望或喜樂較高 - 光明與盼望經文
            selectedCategory = 'lightAndHope';
        } else if (peace > 20) {
            // 平安較高 - 平安與喜樂經文
            selectedCategory = 'peaceAndJoy';
        } else {
            // 隨機選擇一個聖經經文類別
            const categories = ['nativityScriptures', 'angelicMessages', 'lightAndHope', 'loveAndSalvation', 'peaceAndJoy'];
            selectedCategory = categories[Math.floor(Math.random() * categories.length)];
        }
        
        // 從選定類別中隨機選擇一句聖經經文
        const categoryBlessings = randomBlessings[selectedCategory];
        if (categoryBlessings && categoryBlessings.length > 0) {
            return categoryBlessings[Math.floor(Math.random() * categoryBlessings.length)];
        }
        
        return randomBlessings.nativityScriptures[0]; // 備用經文
    }

    // 獲取額外的聖經祝福
    function getExtraBlessings() {
        const extraBlessings = [];
        
        // 隨機添加聖誕特別經文祝福
        if (Math.random() < 0.6) {
            const specialBlessing = christmasSpecialBlessings[Math.floor(Math.random() * christmasSpecialBlessings.length)];
            extraBlessings.push(specialBlessing);
        }
        
        // 隨機添加感恩讚美經文
        if (Math.random() < 0.5) {
            const gratitudeBlessing = gratitudeBlessings[Math.floor(Math.random() * gratitudeBlessings.length)];
            extraBlessings.push(gratitudeBlessing);
        }
        
        return extraBlessings;
    }

    // 獲取沒有收穫時的鼓勵訊息
    function getNoHarvestEncouragement() {
        const encouragements = [
            "沒關係！每一次嘗試都是成長的開始。「凡勞苦擔重擔的人可以到我這裡來，我就使你們得安息。」- 馬太福音 11:28",
            "別灰心！神看重你願意嘗試的心。「你們祈求，就給你們；尋找，就尋見；叩門，就給你們開門。」- 馬太福音 7:7", 
            "失敗是成功之母！「我靠著那加給我力量的，凡事都能做。」- 腓立比書 4:13",
            "再試一次吧！神的恩典夠你用。「我的恩典夠你用的，因為我的能力是在人的軟弱上顯得完全。」- 哥林多後書 12:9",
            "不要放棄！神正在預備美好的事。「我們曉得萬事都互相效力，叫愛神的人得益處。」- 羅馬書 8:28",
            "繼續努力！你的堅持神都看見。「所以，我親愛的弟兄們，你們務要堅固，不可搖動，常常竭力多做主工。」- 哥林多前書 15:58"
        ];
        
        return encouragements[Math.floor(Math.random() * encouragements.length)];
    }

    // 生成完整的屬靈成長報告
    function generateGrowthReport(spiritualStat, completedPhrases = [], collectedChars = []) {
        const totalChars = collectedChars.length;
        const totalGrowth = Object.values(spiritualStat).reduce((sum, val) => sum + (val || 0), 0);
        
        // 檢查是否完全沒有收穫
        if (totalChars === 0 && totalGrowth === 0) {
            return `🌱 ${getNoHarvestEncouragement()}\n\n💡 小提示：\n• 使用方向鍵或點擊下方按鈕控制蛇的移動\n• 收集聖誕字符讓蛇變長並獲得祝福\n• 試著組成「聖誕快樂」、「平安喜樂」等詞句\n• 每個字符都有不同的屬靈意義，慢慢探索吧！\n\n✨ 再試一次，願神的愛與你同在！`;
        }
        
        let report = "";
        
        
        // 只顯示一句智能選擇的聖經經文祝福
        const randomBlessing = getRandomBlessingByCategory(spiritualStat, completedPhrases);
        report += `💝 ${randomBlessing}\n\n`;
        
        
        // 最高成長領域
        const maxGrowthArea = Object.entries(spiritualStat).reduce((max, [key, value]) => {
            return (value || 0) > (max.value || 0) ? { area: key, value } : max;
        }, { area: '', value: 0 });
        
        const areaNames = {
            faith: '信心', love: '愛心', hope: '盼望', 
            peace: '平安', joy: '喜樂', praise: '讚美', 
            wisdom: '智慧', trust: '信靠'
        };
        
        if (maxGrowthArea.value > 0) {
            report += `✨ 你在「${areaNames[maxGrowthArea.area] || maxGrowthArea.area}」方面成長最多！`;
        }
        
        return report;
    }

    // 對外介面 (保持與原版相容)
    function line(tag) {
        return getBlessingLine(tag);
    }

    return { 
        analyze, 
        line,
        getBlessingLine,
        getPhraseBlessing,
        generateGrowthReport,
        getRandomBlessingByCategory,
        getExtraBlessings,
        getNoHarvestEncouragement
    };
})();